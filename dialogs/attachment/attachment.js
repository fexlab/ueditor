    ...
    UploadFile.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                ...
                acceptExtensions = (editor.getOpt('fileAllowFiles') ||
                    [".txt",".doc",".docx",".xls",".xlsx",".ppt",".pptx",".pdf",".odt",".ott",".fodt",".uot",".xml",".dot",".htm",".html",".rtf",".docm",".zip",".rar",".tar",".7z",".tar.gz",".tar.bz",".tar.xz"]).join('').replace(/\./g, ',').replace(/^[,]/, '');;
        ...
        getInsertList: function () {
            var i, link, data, list = [],
                ...
                fileOriginalField = editor.getOpt("fileUploadService")(this, editor).fileOriginalField || 'original',
                fileSrc = '',
                fileSrcFieldKeys = fileSrcField.split('.'),
                fileOriginalFieldKeys = fileOriginalField.split('.');
            ...
            for (i = 0; i < this.fileList.length; i++) {
                ...
                if(fileOriginalFieldKeys.length > 1) {
                    function setFileOriginal(obj, keys, index) {
                        obj = obj[keys[index]];
                        if (index < keys.length - 1) {
                            setFileOriginal(obj, keys, index += 1)
                        } else {
                            fileOriginal = obj;
                        }
                    }

                    setFileOriginal(data, fileOriginalFieldKeys, 0);
                } else {
                    fileOriginal = data[fileOriginalField];
                }

                link = fileSrc;
                list.push({
                    title: fileOriginal || link.substr(link.lastIndexOf('/') + 1),
                    url: prefix + link
                });
            }
            return list;
